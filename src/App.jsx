...
const statusMeta = {
  green: ["dot-green", "Live"],
  live: ["dot-green", "Live"],
  yellow: ["dot-yellow", "Under Maintenance"],
  red: ["dot-red", "Offline"],
  blue: ["dot-blue", "Planning"],
};
...
function Navigation({ archive = false }) {
  ...
  return (
    <nav
      className={`${open ? "menu-open " : ""}${scrolled || archive ? "is-scrolled" : ""}`}
...
function Hero() {
  return (
    <section id="hero">
      ...
      <div id="hero-content">
        <p id="hero-eyebrow">
          USC CS Student &amp; Sysadmin{" "}
          <span>Served from Ubuntu 24.04.4 LTS</span>
        </p>
        <h1 id="hero-name">
          Nathaniel Ryan <span>Ponce</span>
        </h1>
        <p id="hero-role">
          <strong>SysAdmin &amp; DevSecOps</strong> · Computer Science
        </p>
        <p id="hero-tagline">
          Building resilient systems and clean software — from bare-metal
          servers to full-stack web. Currently studying at USC while keeping the
          infrastructure running and the servers humming.
        </p>
        <div id="hero-buttons">
      ...
      <div id="hero-symbols" aria-hidden="true">
      ...
      <div id="hero-scroll" aria-hidden="true">
  ...
  useEffect(() => {
    ...
    const play = async () => {
      ...
      if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
        setLines(
          terminalLines.map(([command, output]) => ({
            command,
            output,
            typing: null,
          })),
      ...
      for (const [command, output] of terminalLines) {
        if (cancelled) return;
        const index = terminalLines.findIndex(([name]) => name === command);
        setLines((current) => [
          ...current,
          { command: "", output: "", typing: "command" },
        ]);
        await type(index, "command", command, 55);
        await wait(280);
        await type(index, "output", output, 34);
        await wait(420);
      }
    };
  ...
  return (
    <aside ref={ref} className="about-terminal" aria-label="Current focus">
      ...
      <div
        ...
        id="about-terminal-output"
        aria-live="polite"
      >
        {lines.map((line, index) => (
          <div key={terminalLines[index][0]}>
            <p
              className={`terminal-line${line.typing === "command" ? " terminal-caret" : ""}`}
            >
              <span>$</span> {line.command}
            </p>
            {(line.output || line.typing === "output") && (
              <p
                className={`terminal-output${line.typing === "output" ? " terminal-caret" : ""}`}
              >
                {line.output}
              </p>
            )}
...
function Reveal({
  ...
  return (
    <Element
      ref={ref}
      className={`${className} projects-reveal-item${visible ? " projects-reveal-visible" : ""}`}
      style={{ "--projects-reveal-delay": `${delay}ms` }}
      {...props}
    >
      {children}
    </Element>
  ...
  return (
    <section className="stack-showcase" aria-labelledby="stack-title">
      <div className="stack-showcase-header">
        ...
        <h3 id="stack-title">Technologies I work with and learn</h3>
      </div>
      <div className="stack-carousel" aria-label="Technology stack carousel">
        <div className="stack-track" id="stack-track">
...
function About() {
  const cards = [
    ...
    [
      "02",
      "Software",
      "I build with HTML, CSS, JavaScript, and backend concepts while expanding toward full-stack development.",
    ],
    [
      "03",
      "Growth",
      "I treat each project as a chance to improve my structure, project design management, and problem-solving process.",
    ],
  ];
  return (
    <section id="about" aria-labelledby="about-title">
      <div className="section-container">
        <p className="section-kicker">About / Background</p>
        <div className="about-grid">
          <div className="about-copy">
            <h2 id="about-title">
              Hi, call me <span>Nathan</span>
            </h2>
            <p>
              My name is Nathaniel Ryan Ponce, a Computer Science student at the
              University of San Carlos with a growing focus on system
              administration, DevSecOps, and full-stack web development.
            </p>
            <p>
              I like working close to the machine: Linux servers, deployment
              flows, web infrastructure, and the practical side of keeping
              things online. I am expanding my depth in cybersecurity so the
              systems I build are as secure as they are stable.
            </p>
            <p>
              This portfolio is both a personal website and a live project,
              served from my own Ubuntu Server setup with an AMD Ryzen 3 4300GE
              and 16 GB DDR5 RAM.
            </p>
        ...
        <div className="about-focus-grid" aria-label="Technical focus areas">
          {cards.map(([number, title, text]) => (
            <article className="about-card" key={number}>
      ...
      <div className="project-card">
        ...
        <div className="project-card-footer">
          ...
          <span className="status-badge">
            ...
            {project.statusLabel || defaultLabel}
          </span>
        </div>
      </div>
    </Reveal>
...
function Projects({ projects, archive = false }) {
  ...
  return (
    <section
      id="projects"
      className={archive ? "projects-archive" : ""}
      aria-labelledby={archive ? "projects-title" : undefined}
    >
      <div className="section-container">
        <Reveal className="projects-heading">
          {archive ? (
            ...
            <>
              ...
              <h2>Hosted builds with sharp edges and practical systems.</h2>
            </>
          )}
        </Reveal>
        <div
          className="projects-list"
          aria-label={archive ? "All projects" : "Featured projects"}
        >
          {visible.map((project) => (
...
function Contact() {
  return (
    ...
    >
      <canvas id="contact-canvas" aria-hidden="true" />
      <div className="section-container contact-container">
        <div className="contact-heading">
          ...
          <h2 id="contact-title">
            ...
            <br />
            Engineered to <span>execute</span>
          </h2>
          <p>
            Have a project idea, internship opportunity, or systems problem that
            needs a steady pair of hands? I am open to conversations around
            system builds, infrastructure, security-minded workflows, and
            student-led technical projects.
          </p>
        </div>
        <div className="contact-layout">
          <div className="contact-primary">
            <div className="contact-circle-mark" />
            <span className="contact-label">Available for Internship</span>
            <a
              className="contact-email"
              href="mailto:nathanielryanponce@gmail.com"
            >
              <MailIcon />
              nathanielryanponce@gmail.com
            </a>
            <p>
              Best for collaboration ideas, build requests, school projects,
              infrastructure questions, and opportunities where practical
          ...
          <div className="contact-methods" aria-label="Contact links">
            ...
            >
              ...
              <span>
                ...
                <small>Code, experiments, and hosted projects</small>
              </span>
            </a>
            <a
            ...
            >
              ...
              <span>
                ...
                <small>Professional updates and opportunities</small>
              </span>
            </a>
        ...
        <div className="contact-bottom">
          <div className="contact-legal">
            <p>© 2026 Nathaniel Ryan Ponce. All rights reserved.</p>
            <p>Designed, built, and served by Nathaniel Ponce.</p>
          </div>
