...
Generated or historical material is intentionally outside the application path: `node_modules/`, `dist/`, `graphify-out/`, `docs/`, and `import/` should not be used as source. Vite regenerates `dist/`; Graphify regenerates `graphify-out/`.
...
`graphify-out/` is a generated architecture index, not application source. Rebuild it after structural changes with:

```bash
graphify . --update
```

Use `graphify query "your architecture question"` to explore the resulting graph. Do not hand-edit generated Graphify outputs.
